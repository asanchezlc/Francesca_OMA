...
NPERSEG_FDD = 1024  # 4096 results in a good balance between freq. res. and averaging

# Parameters for peak picking
PP_FREQLIM = (30, 400)     # Frequency range [Hz]; use None to analyze the full range
...
PP_MIN_PROMINENCE = 2.7      # Minimum peak prominence. In dB if PP_USE_DB=True
PP_MIN_DISTANCE_HZ = 4    # Minimum distance between peaks [Hz]
...
plt.legend()
plt.tight_layout()
plt.savefig(figures_dir / f"{OMA_METHOD}_TRUSS_peak_picking.png")
...
MPE_DF = 3          # Frequency bandwidth around each selected peak [Hz], only for FDD
if OMA_METHOD == "FDD":
    Pali_ss.mpe(
        "FDD",
